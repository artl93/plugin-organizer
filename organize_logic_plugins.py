...
def match_override(
    plugin: Plugin, vendor: str | None, override: dict[str, Any]
) -> bool:
    for key, value in override.items():
        if key == "category":
            continue
        if key == "name":
            if normalize(plugin.name) != normalize(str(value)):
                return False
        elif key == "bundle_id":
            ...
            if not re.search(str(value), plugin.name, re.IGNORECASE):
                return False
    return True
...
def categorize_plugin(
    ...
    for override in mapping.get("overrides", []):
        if match_override(plugin, vendor, override):
            return override["category"], vendor

    if vendor:
        vendor_rules = mapping.get("vendor_rules", {}).get(vendor, [])
        category = match_rules(plugin.name, vendor_rules)
        if category:
            return category, vendor

    category = match_rules(plugin.name, mapping.get("rules", []))
    if category:
        return category, vendor
...
def main() -> None:
    ...
    for plugin in plugins:
        category, vendor = categorize_plugin(plugin, mapping, vendor_aliases)
        results.append(
            {
                "name": plugin.name,
    ...
    if args.apply:
        ...
        backup_dir = backup_tags(tags_dir, Path(args.backup_dir).expanduser())
        ...
        for plugin in plugins:
            category = next(
                item["category"]
                for item in results
                if item["tagset"] == plugin.tagset_name()
            )
            if not write_tagset(tags_dir, plugin, category, args.merge_tags):
